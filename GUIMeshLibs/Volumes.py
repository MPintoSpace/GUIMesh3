...
def SaveVolumeProperties(list_of_objects):
    filename=tkinter.simpledialog.askstring("","Please type a filename: ")
    ...
    if (path_to_file[-4:]==".csv"):
        ...
        if (len(properties)==len(list_of_objects)):
            ...
            for line in properties:
                ...
                counter+=1
            tkinter.messagebox.showinfo("Message", "Load complete. Check log for errors.")

        else:
            tkinter.messagebox.showinfo("Warning", "Your list has incorrect size")
    else:
        tkinter.messagebox.showinfo("Error", "Must be csv file.")
