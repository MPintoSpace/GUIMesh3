...
def Pressed_Load_Mats_Dir():
    ...
    New_Mats_List=Materials.Load_Materials()#See Materials Library
    if (New_Mats_List!=0):
        if len(New_Mats_List)==0:
            print("No materials found in Database")
            tkinter.messagebox.showinfo("Error", '"No materials found in Database.') 
        ...
        tkinter.messagebox.showinfo("Error", '"Materials" folder not found.')
...
def Pressed_Save_Mats():
    global MatManager_Material_List
    success=Materials.Save_Materials(MatManager_Material_List) #See Material Library
    if (success==0):
        tkinter.messagebox.showinfo("Error", '"Materials" folder not found.')
...
def Pressed_Load_Mat():
    ...
    try:
        path_to_material = tkinter.filedialog.askopenfilename()
    except ValueError:
        return 0

    newmat=Materials.Load_Material(path_to_material)#See Material Library
    if newmat==0:
        print("Error: Material was not loaded.")
    else:
        if newmat.Name in temp_name_list:
            print("Error: "+i.Name+" is already used.")
            tkinter.messagebox.showinfo("Error", "Error: "+i.Name+" is already used.")    
        else:
...
def Save_Material(name,density,elements,fractions):
    ...
    if name.get() in temp_name_list:
        ...
        tkinter.messagebox.showinfo("Error", "Error: "+name.get()+" is already used.") 
        return 0
    check_fractions=0.0
    try:
        float(density.get())<=0
        for i in range(0,len(elements)):
            try:
                ...
                if elements[i].get() in ele_db_names:
                    ...
                    tkinter.messagebox.showinfo("Error", "Chosen Element: "+elements[i].get()+" not in DB.") 
                    return 0
            except ValueError:
                print("Error: Fraction #"+str(i+1)+" not a float.")
                tkinter.messagebox.showinfo("Error", "Error: Fraction #"+str(i+1)+"not a float.") 
                return 0
    except ValueError:
        print("Error: Material density either not a float or <1.")
        tkinter.messagebox.showinfo("Error","Material density either not a float or <1.") 
        return 0
    if(check_fractions==1.0):
        newmat=Materials.Material(name.get(),float(density.get()),len(elements),newmat_elements_list)
        MatManager_Material_List.append(newmat)
        mylist_mat_db.insert(tk.END, newmat.Name)
        top.destroy()
    else:
        print("Error: Element fraction sum different than 1.")
        tkinter.messagebox.showinfo("Error","Error: Element fraction sum different than 1.")
        return 0
...
def Pressed_Create_Mat():
    ...
    for i in MatManager_Material_List:
        print(i.Name)
    total_elements=0.0
    n_comp=tkinter.simpledialog.askinteger('Create Material ', 'Choose the number of elements.')
    if n_comp<1:
        tkinter.messagebox.showinfo("Error", "Error: Number of elements must be >0.") 
        return 0
    #new window
    ...
    for i in range(0,n_comp):
        labels_name.append(tk.Label(top, text="Element "+ str(i+1)+" name",font=("Helvetica",12)).grid(row=round(4+2*(i%8)),column=round(2*(i/8)+1),sticky="W"))
        entrys_name.append(tk.Entry(top, bd =5))
        entrys_name[i].grid(row=round(4+2*(i%8)), column=round(2*(i/8)+1))
        labels_fraction.append(tk.Label(top, text="Element "+ str(i+1)+" fraction",font=("Helvetica",12)).grid(row=round(5+2*(i%8)),column=round(2*(i/8)+1),sticky="W"))
        entrys_fraction.append(tk.Entry(top, bd =5))
        entrys_fraction[i].grid(row=round(5+2*(i%8)), column=round(2*(i/8)+1))
