...
def Pressed_Find_FreeCAD_Dir():
    ...
    try:
        ...
        tkinter.messagebox.showinfo("Success", "FreeCAD loaded.")
        ...
        if r:
            ...
            tkinter.messagebox.showinfo("Warning", "There was an error loading FreeCAD.")
...
def Pressed_Read_STEP():
    ...
    if (FreeCAD_status=="FreeCAD loaded"):
        ...
        #Creates new object list
            ...
            file_status=1
            tkinter.messagebox.showinfo("Success", "STEP file loaded.")
        ...
        tkinter.messagebox.showinfo("Error", "FreeCAD not found.")
...
def Save_World_Button(x,y,z,top):
    ...
    try:
        ...
        tkinter.messagebox.showinfo("Error", "All variables must be float values larger than 0.")
        return 0
...
def Pressed_Change_Material():
    ...
    newMaterial=tkinter.simpledialog.askstring('Material ', 'Select new Material');
    ...
    if check_mat==0:
        tkinter.messagebox.showinfo("Error", "Material not found in database.")
...
def Pressed_Change_MMD():
    ...
    newMMD=tkinter.simpledialog.askfloat('MMD ', 'Select new MMD value.');
    try:
        if float(newMMD)>0:
            print("MMD ok")
            if set_all_var.get()==0: #Selected volume
            ...
            tkinter.messagebox.showinfo("Error", "MMD must be greater than 0.")
            print("Chosen value <= 0")
    except:
        tkinter.messagebox.showinfo("Error", "Value not a float.")
...
def Pressed_Change_GDML_Option():
    ...
    newGDMLoption=tkinter.simpledialog.askinteger('Write Option', 'Select Write Option.'); #>=1 write - <1 do not write
    if newGDMLoption<=0:
        newGDMLoption=0
    else:
        newGDMLoption=1
